...
const HistoryCard = async ({ limit, userId }: Props) => {
  const game = await prisma.game.findMany({ where: { userId }, take: limit, orderBy: { timeStarted: "desc" } });
  return (
        ...
        return (
          <div className="flex items-center justify-between" key={game.id}>
            <div className="flex items-center">
              {game.gameType === "mcq" ? (<CopyCheck className="mr-3" />) : (<Edit2 className="mr-3" />)}
              <div className="ml-4 space-y-1">
                ...
                <p className="text-sm text-muted-foreground">{game.gameType === "mcq" ? "Multiple Choice Question" : "Open Ended"}</p>
              </div>
            </div>
          </div>
        )
